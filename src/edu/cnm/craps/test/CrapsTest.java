package edu.cnm.craps.test;

import edu.cnm.craps.model.Craps;
...
class CrapsTest {
  ...
  void play() {
    ...
    Assertions.assertNotEquals(0, rolls.size());
    ...
    switch (state) {
      case WIN:
        ...
        break;
      case LOSS:
        //TODO Check for (and fail if found) a firstRoll of 7 or 11.
        if (rolls.size() == 1) {
          Assertions.assertTrue(firstRoll == 2 || firstRoll == 3 || firstRoll == 12);

        } else {
          Assertions.assertEquals(7, lastRoll);
          //TODO Check for (and fail if found) firstRoll occurs sometimes after firstRoll.
        }
        break;
      default:
